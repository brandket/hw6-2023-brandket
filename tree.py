    ...
    def predict(self, x):
        ...
        for i, row in enumerate(x):
            ...
            while not isinstance(node, Leaf):
                if row[node.feature] >= node.value: #значение больше значения в промежуточном листе
...
class DecisionTreeRegressor(BaseDecisionTree):
    ...
    def criteria(self, y):
        return np.var(y) #информативность для жадного алгоритма это - дисперсия целевой переменной
    
    def leaf_value(self, y):
        return np.mean(y) #в регрессии просто присваеваем среднее в листе
